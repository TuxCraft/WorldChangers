...
public class WorldGenGhastHive extends WorldGenerator {
	...
	public static final WeightedRandomChestContent[] ghastHiveSmallChestContents = new WeightedRandomChestContent[] {
			new WeightedRandomChestContent(Item.blazePowder.itemID, 2, 0, 1, 6),
			new WeightedRandomChestContent(Item.blazeRod.itemID, 1, 0, 1, 4),
			new WeightedRandomChestContent(Item.netherQuartz.itemID, 3, 0, 1, 10),
			new WeightedRandomChestContent(new ItemStack(Item.skull, 1, 1), 1, 5, 6),
			new WeightedRandomChestContent(Item.gunpowder.itemID, 2, 0, 1, 8),
			new WeightedRandomChestContent(Item.ghastTear.itemID, 2, 1, 5, 10)
			};
					...
					if (Math.sqrt(Math.pow(i, 2) + Math.pow(j, 2)
						...
						if (random <= 1) {
							...
							if (random2 == 1) {
								...
								if (tileentitychest != null) {
									WCChestGenHooks info = WCChestGenHooks
											.getInfo(WCChestGenHooks.HIVE_SMALL);
									WeightedRandomChestContent
											.generateChestContents(rand,
													info.getItems(rand),
													tileentitychest,
													info.getCount(rand));
								}
