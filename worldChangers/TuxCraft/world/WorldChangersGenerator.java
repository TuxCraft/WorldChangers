...
public class WorldChangersGenerator implements IWorldGenerator {
	...
	private boolean generateVolcano(World world, Random random, int chunkX,
			int chunkZ) {
		...
		if ((world.getBiomeGenForCoords(x, z).minHeight > 0.0F)
				&& (world.getBiomeGenForCoords(x, z).maxHeight < 0.4F)) {
			if (random.nextInt(spawnRate) == 1) {
				long start = System.currentTimeMillis();
				new WorldGenVolcano(true).generate(world, random, x, y, z);
				System.out
						.println("Volcono spawned at "
								+ x
								+ " "
								+ y
								+ " "
								+ z
								+ " In biome "
								+ String.valueOf(world.getBiomeGenForCoords(x,
										z).biomeName) + ": Took "
